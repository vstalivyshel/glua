use std::{
    ...
    env,
    ffi::CString,
    io::{Read, BufRead, BufWriter, BufReader, Result, Write, self},
...
fn run() -> Result<()> {
    ...
    if args_len < 1 {
        return Ok(eprintln!("fail wrong arument count"));
    }
    ...
    match args.pop_front().unwrap().as_str() {
        "pipe" => if args_len >= 1 {
			...
			daemonize()?;
			let fifo: fs::File;
			match fs::OpenOptions::new().write(true).open(&fifo_path) {
                Err(_) => return Ok(()),
                Ok(f) => fifo = f,
			}
